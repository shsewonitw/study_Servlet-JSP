...
<!-- 
<c:if test="${ not empty sessionScope.login_member and not empty applicationScope.login_member_count }">
...
<c:if test="${ not result }" >	
	<li><a href='<%=request.getContextPath()%>/auth/member.do'>${sessionScope.login_member.name }님의 개인정보 보기</a></li>
	<li><a href='<%=request.getContextPath()%>/auth/logout.do'>로그아웃</a></li>
	<li><a href='<%=request.getContextPath()%>/auth/article.do'>자유게시판</a></li>
</c:if>
</ul>
